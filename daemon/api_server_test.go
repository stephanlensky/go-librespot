...
func TestApiStream(t *testing.T) {
	...
	// Range support is why this endpoint is served with http.ServeContent
	// rather than going through handleRequest like every other handler.
	t.Run("serves a byte range", func(t *testing.T) {
		...
		req, err := http.NewRequest(http.MethodGet, ts.url+"/player/stream?uri=spotify:track:4iV5W9uYEdYUVa79Axb7Rh", nil)
		require.NoError(t, err)
		...
		require.Equal(t, "bytes 10-14/21", resp.Header.Get("Content-Range"))
		require.Equal(t, "audio", body(t, resp))
	})

	t.Run("reports a daemon error as its mapped status", func(t *testing.T) {
		ts := newTestServer(t, func(ApiRequest) (any, error) {
